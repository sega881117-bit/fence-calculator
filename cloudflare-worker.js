...
function quoteDocument(quote) {
  ...
  const html = `<article class="quote-document" data-layout-version="${QUOTE_LAYOUT_VERSION}">
    ...
    <table><thead><tr><th>Работы и материалы</th><th>Ед. изм.</th><th>Кол‑во</th><th>Цена, руб.</th><th>Сумма, руб.</th></tr></thead><tbody>${rows}</tbody><tfoot><tr><th colspan="3">Итого</th><td class="quote-document__total-caption">Включая материалы и работы</td><td class="quote-document__total-value"><strong>${rub(quote.total)}</strong></td></tr></tfoot></table>
